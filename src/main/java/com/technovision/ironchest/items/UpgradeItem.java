...
public class UpgradeItem extends Item {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (blockEntity != null) {
            ...
            Direction chestFacing = world.getBlockState(blockPos).get(ChestBlock.FACING);
            ...
            NbtCompound oldChest = new NbtCompound();
            chest.writeNbt(oldChest);
            world.setBlockState(blockPos, blockState, 3);
            world.updateListeners(blockPos, blockState, blockState, 3);

            world.getBlockEntity(blockPos).readNbt(oldChest);
